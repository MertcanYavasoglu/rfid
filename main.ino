...
// ADD NEW UIDs IF NEW ADMIN CARDS
...
void setup() {
  ...
  configTime(3 * 3600, 0,           // UTC+3 (Istanbul) için ofset
             "pool.ntp.org",
             "time.nist.gov");
...
void handleUIDLearn(MFRC522 &r){
  ...
  for(int i = 0; i < ADMIN_COUNT; i++){
    if(compareUID(r.uid.uidByte, ADMIN_CARDS[i])){
      // Admin kartı, öğrenme adımını atla
      r.PICC_HaltA();
      delay(300);
      return;
    }
...
String getFormattedTime(){
  ...
  if(!getLocalTime(&timeinfo)){
    return String("0000-00-00 00:00:00"); // zaman yoksa fallback
  }
  char buf[20];
